...
export default function VendorReputationBadge({
  vendorId,
}: {
  vendorId: string;
}) {
    ...
    rep.completed_orders >= 100 && rep.avg_rating >= 4.5
      ...
      : rep.completed_orders >= 25 && rep.avg_rating >= 4
        ...
        : rep.completed_orders >= 5
          ...
          : {
              label: "ROOKIE",
              color: "text-muted-foreground border-border bg-secondary/30",
            };

  return (
    <div className="flex items-center gap-2 text-[10px] font-mono">
      <span
        className={`px-1.5 py-0.5 rounded border ${tier.color} flex items-center gap-1`}
      >
