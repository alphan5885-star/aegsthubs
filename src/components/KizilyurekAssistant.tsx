  ...
  const send = async (text: string) => {
    ...
    try {
              ...
              if (generating) {
                ...
                setMessages((p) => {
                  ...
                  if (last?.role === "assistant")
                    return [...p.slice(0, -1), { ...last, content: acc }];
      ...
      setMessages((p) => [
        ...
        {
          role: "assistant",
          content: "⚠️ BAĞLANTI_HATASI: SİSTEM_YANIT_VERMİYOR.",
        },
      ]);
    } finally {
      setLoading(false);
          ...
          >
            ...
            <div className="px-8 py-2 bg-red-600/5 flex items-center justify-between border-b border-white/5 text-[7px] font-black uppercase tracking-[0.2em] text-red-900">
              <div className="flex items-center gap-2">
                <Activity className="w-3 h-3" /> SYNC: 99.8%
              </div>
              <div className="flex items-center gap-2">
            ...
            >
                ...
                <div className="space-y-8 relative z-10">
                  <div className="p-6 rounded-[32px] border border-red-600/20 bg-red-600/[0.03] space-y-3">
                    ...
                    <p className="text-[9px] text-zinc-600 font-bold leading-relaxed uppercase">
                      ASLA İSİM, ADRES VEYA OPERASYONEL VERİ PAYLAŞMAYIN. BU
                      KANAL DIŞ AI SERVİSLERİYLE SENKRONİZE ÇALIŞIR.
                    </p>
                  </div>
                  <div className="space-y-4">
                    <div className="text-[9px] text-zinc-800 font-black uppercase tracking-[0.4em]">
                      HIZLI_KOMUTLAR
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {QUICK.map((q) => (
                        <button
                          key={q}
                          onClick={() => send(q)}
                          className="px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-[9px] font-black text-zinc-500 hover:text-red-600 hover:border-red-600/40 transition-all uppercase"
                        >
                          {q}
                        </button>
                      ))}
              ...
              {messages.map((m, i) => (
                ...
                >
                  <div
                    className={`max-w-[85%] p-5 rounded-[24px] text-[11px] leading-relaxed font-black uppercase tracking-widest ${m.role === "user" ? "bg-red-600 text-white shadow-[0_10px_20px_rgba(255,0,0,0.2)]" : "bg-white/5 text-zinc-400 border border-white/5"}`}
                  >
                    {m.role === "assistant" ? (
              ...
              {loading && (
                <div className="flex items-center gap-4 px-2">
                  ...
                  <span className="text-[8px] font-black text-red-900 uppercase tracking-widest">
                    VERİ_İŞLENİYOR...
                  </span>
                </div>
              )}
