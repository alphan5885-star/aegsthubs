...
export default function CommandPalette() {
  ...
  const pages: Item[] = [
    ...
    {
      id: "p6",
      label: "GÜVENLİK_MERKEZİ",
      icon: Lock,
      action: () => go("/security-settings"),
      group: "SİSTEM_DİZİNİ",
    },
    {
      id: "p7",
      label: "KİMLİK_PROFİLİ",
      icon: User,
      action: () => go("/profile"),
      group: "SİSTEM_DİZİNİ",
    },
    {
      id: "p9",
      label: "OPERASYON_PANELİ",
      icon: Store,
      action: () => go("/vendor-dashboard"),
      group: "SİSTEM_DİZİNİ",
    },
  ];
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-6 px-10 py-8 border-b border-white/5 bg-white/[0.02]">
              <Search
                className={`w-6 h-6 ${analyzing ? "text-red-600 animate-pulse" : "text-zinc-700"}`}
              />
              <input
                autoFocus
                value={query}
              ...
              <div className="flex items-center gap-3">
                <div className="text-[10px] text-zinc-800 font-black uppercase tracking-widest hidden md:block">
                  RESOLVER_v4.2
                </div>
                <div className="w-[1px] h-4 bg-white/5 hidden md:block" />
                <kbd className="text-[10px] font-black text-zinc-700 border border-white/5 px-2 py-1 rounded-lg uppercase">
                  ESC
                </kbd>
              </div>
            </div>
              ...
              {Object.entries(grouped).map(([group, items]) => (
                <div key={group} className="space-y-2 mb-8">
                  <div className="px-10 flex items-center gap-4">
                    <div className="text-[9px] font-black text-red-900 uppercase tracking-[0.4em]">
                      [{group}]
                    </div>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        {active && (
                          <div className="absolute left-0 w-1.5 h-full bg-red-600 shadow-[0_0_15px_#ff0000]" />
                        )}
                        <div
                          className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${active ? "bg-red-600 border-red-600 text-white shadow-[0_0_15px_rgba(255,0,0,0.3)]" : "bg-white/5 border-white/5 text-zinc-700 group-hover:text-white"}`}
                        >
                          <item.icon className="w-5 h-5" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div
                            className={`text-sm font-black uppercase tracking-widest ${active ? "text-white" : "text-zinc-500 group-hover:text-zinc-300"}`}
                          >
                            {item.label}
                          </div>
                          {item.sub && (
                            <div className="text-[9px] font-bold text-zinc-800 uppercase tracking-widest mt-1">
                              {item.sub}
                            </div>
                          )}
                        </div>
                        {active && (
                          <Activity className="w-4 h-4 text-red-900 animate-pulse" />
                        )}
                      </button>
                    );
                  })}
