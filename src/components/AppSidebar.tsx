...
export default function AppSidebar() {
  ...
  if (role !== "admin" && role !== "vendor") {
    accountLinks.push({ to: "/vendor/bond", labelKey: "vendorApplication", icon: Shield });
  }
    ...
    return (
      <Link
        ...
        className={`group flex items-center gap-4 px-8 py-3.5 transition-all duration-500 relative ${
          active ? "text-white bg-white/[0.04]" : "text-zinc-700 hover:text-white hover:bg-white/[0.01]"
      ...
      >
        ...
        >
          <link.icon className={`w-5 h-5 transition-all duration-300 ${active ? "text-red-600 drop-shadow-[0_0_12px_#ff0000]" : "group-hover:text-red-600 group-hover:drop-shadow-[0_0_8px_rgba(255,0,0,0.4)]"}`} />
        </motion.div>
        {!collapsed && (
          <span className={`text-[10px] font-black uppercase tracking-[0.3em] transition-all duration-300 ${active ? "text-white" : "group-hover:tracking-[0.4em]"}`}>
            {t(link.labelKey as any)}
          </span>
        )}
        {active && !collapsed && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="ml-auto w-1.5 h-1.5 rounded-full bg-red-600 shadow-[0_0_10px_#ff0000]" />
        )}
      </Link>
    );
  };
  ...
  const renderSection = (title: string, links: LinkDef[]) => {
    ...
    return (
      <div className="mb-4">
        <div className="px-8 py-2 text-[8px] font-black text-zinc-800 uppercase tracking-[0.4em]">{title}</div>
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen ${collapsed ? "w-[80px]" : "w-[280px]"} bg-[#020202] border-r border-white/5 flex flex-col z-50 transition-all duration-500 ease-in-out`}>
      {/* Sidebar Header */}
      <div className="py-5 px-6 border-b border-white/5 flex flex-col items-center gap-4">
        <div className="w-full flex items-center justify-between">
           <button onClick={() => updateSettings({ sidebarCollapsed: !collapsed })} className="text-zinc-800 hover:text-white transition-colors p-1">
        ...
        <div className="cursor-pointer group relative" onClick={() => navigate("/market")}>
        ...
        {!collapsed && (
          <div className="text-center space-y-1">
            ...
            <div className="text-[7.5px] text-zinc-600 font-bold uppercase tracking-[0.4em] flex items-center gap-1.5 justify-center leading-none">
               <Activity className="w-2.5 h-2.5 animate-pulse text-red-950" /> NODE_CONNECTED
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 px-6">
         ...
         >
           ...
           {!collapsed && <span className="text-[9px] font-black uppercase tracking-widest flex-1 text-left">_SEARCH</span>}
         </button>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto no-scrollbar py-2">
        ...
        <div className="mt-4 px-8 space-y-2">
           <button onClick={() => window.dispatchEvent(new CustomEvent("kizilyurek:toggle"))} className="w-full flex items-center gap-3 text-[10px] font-black text-red-900 hover:text-red-600 uppercase tracking-widest transition-colors">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-[7px] px-1.5 py-0.5 rounded font-black tracking-widest border uppercase ${
                  role === "admin"
              ...
              <div className="text-[8px] text-zinc-600 font-bold uppercase tracking-wider flex items-center gap-1.5">
                ID: <span className="text-zinc-500 font-mono">{user?.id.slice(0, 8).toUpperCase()}</span>
              </div>
            </div>
          )}
        </div>
