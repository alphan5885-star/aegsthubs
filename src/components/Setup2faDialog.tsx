...
import { X, ShieldAlert, Key, Clipboard, Check, Lock, ShieldCheck } from "lucide-react";
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4 font-mono select-none">
      <div className="relative w-full max-w-[500px] bg-[#020202]/95 border-2 border-red-600/30 rounded-[32px] p-8 shadow-[0_0_50px_rgba(255,0,0,0.15)] overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-white/[0.04] pb-4 mb-6 relative">
          ...
          <div>
            <span className="text-[6px] text-red-500 font-black tracking-[0.3em] uppercase">SECURE_ENCLAVE_v2</span>
            <h2 className="text-sm font-black text-white uppercase tracking-wider">2FA (TOTP) AKTİVASYONU</h2>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center gap-2 mb-6 text-[8px] font-black uppercase text-zinc-600 border border-white/[0.03] bg-white/[0.01] rounded-2xl p-2.5">
          ...
          <span className={step === 2 ? "text-red-500" : step > 2 ? "text-green-500" : ""}>2. DOĞRULAMA</span>
          <span className="w-4 h-[1px] bg-zinc-800" />
          <span className={step === 3 ? "text-green-500" : ""}>3. KORUMA AKTİF</span>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-wide leading-relaxed">
                Google Authenticator veya Aegis gibi bir 2FA uygulamasını açıp aşağıdaki QR kodu taratın veya gizli anahtarı manuel olarak ekleyin.
              </p>
              ...
              <div className="bg-[#050505] border border-white/5 rounded-2xl p-4 flex items-center justify-between gap-4">
                <div className="truncate">
                  <span className="text-[7px] text-zinc-600 font-black block uppercase tracking-widest">GİZLİ_ANAHTAR_KODU</span>
                  <span className="text-white text-xs font-black tracking-widest uppercase">{secretKey}</span>
                </div>
                <button
                  type="button"
          ...
          {step === 2 && (
            ...
            >
              <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-wide leading-relaxed text-center">
                2FA uygulamanızın ürettiği 6 haneli tek kullanımlık geçici güvenlik kodunu girin.
              </p>
          ...
          {step === 3 && (
            ...
            >
              ...
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-wider mb-1">MFA AKTİF EDİLDİ</h3>
                <p className="text-[8px] text-zinc-500 font-bold uppercase tracking-widest">HESABINIZ ÜST DÜZEY KORUMA ALTINDA!</p>
              </div>

              {/* Recovery Backup Codes */}
              <div className="bg-[#050505] border border-white/5 rounded-2xl p-4 text-left space-y-3">
                <div>
                  <span className="text-[7px] text-red-500 font-black uppercase tracking-widest block">⚠️ ACİL KURTARMA KODLARI</span>
                  <span className="text-[7px] text-zinc-600 font-bold uppercase tracking-wider block mt-0.5 leading-relaxed">
                    Telefonunuzu kaybetmeniz durumunda sisteme erişmek için bunları saklayın:
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-[9px] font-black tracking-wider text-zinc-300">
                  {backupCodes.map((code, idx) => (
                    <div key={idx} className="bg-white/[0.01] border border-white/[0.02] p-2 rounded-xl text-center select-all">
                      {code}
                    </div>
                  ))}
                </div>
