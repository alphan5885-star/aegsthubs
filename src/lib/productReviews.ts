  ...
  const seed: ProductReview[] = [
    {
      ...
      rating: 5,
      comment:
        "MÜKEMMEL BİR ÜRÜN, HIZLI TESLİMAT VE TAMAMEN ŞİFRELİ SEVKİYAT. ŞİDDETLE TAVSİYE EDERİM!",
    ...
    },
    {
      ...
      rating: 4,
      comment:
        "İletişim harikaydı, teslimat sorunsuz ve gizlilik üst düzeyde sağlandı. Puanım 4/5.",
      username: "CYBER_NOMAD",
      created_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
    },
  ];
  localStorage.setItem(
    `aeigs_product_reviews_${productId}`,
    JSON.stringify(seed),
  );
...
const updateProductAverageRating = (
  productId: string,
...
export const getGlobalProductAverages = (): Record<
  string,
  { avg: number; count: number }
> => {
