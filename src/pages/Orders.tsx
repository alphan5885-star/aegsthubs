...
import { confirmDeliveryFn } from "@/lib/escrowFns";
import {
  CheckCircle,
  Star,
  Truck,
...
export default function Orders() {
    ...
    const fetch = async () => {
      ...
      try {
        const query = supabase
          .from("orders")
          .select(
            "id, amount, status, delivery_method, created_at, products:product_id(name, image_url, image_emoji, type, vendor_id)",
          )
        ...
        if (!isMounted.current) return;
        if (disputes)
          setDisputedOrders(new Set(disputes.map((d: any) => d.order_id)));
  ...
  const confirmDelivery = async (orderId: string) => {
    try {
      if (!user) return;
      await confirmDeliveryFn({ data: { orderId, buyerId: user.id } });
      toast.success(t("orders.confirmDelivery"));
      setOrders((prev) =>
        prev.map((o) => (o.id === orderId ? { ...o, status: "completed" } : o)),
      );
    } catch (error: any) {
      toast.error(error.message || t("err.generic"));
    }
  };
        ...
        <div className="glass-card rounded-lg">
          <EmptyState
            ...
            action={
              <Link
                to="/market"
                className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-xs font-mono rounded hover:opacity-90 transition"
              >
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground font-mono flex items-center gap-2 flex-wrap">
                      {new Date(o.created_at).toLocaleDateString()} • {o.amount}{" "}
                      LTC
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {["paid", "shipped", "delivered", "completed"].includes(
                    o.status,
                  ) && (
                    <OrderChatButton
                      orderId={o.id}
                      label={t("orders.chatBtn")}
                    />
                  )}
                  {role === "buyer" && o.status === "pending" && (
                    <>
                      <Link
                        to={`/payment/${o.id}`}
                        className="flex items-center gap-1 px-2 py-1 bg-blue-600 text-white text-[10px] font-mono rounded hover:bg-blue-700 cursor-pointer"
                        onClick={(e: React.MouseEvent<HTMLAnchorElement>) => e.stopPropagation()}
                      >
                        <ShoppingCart className="w-3 h-3" /> Öde
                      </Link>
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 bg-red-600/80 text-white text-[10px] font-mono rounded hover:bg-red-700 cursor-pointer"
                      >
                  ...
                  {role === "buyer" &&
                    (o.status === "shipped" || o.status === "escrow_locked") &&
                    !disputedOrders.has(o.id) && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="flex items-center gap-1 px-2 py-1 bg-green-600 text-white text-[10px] font-mono rounded hover:bg-green-700 cursor-pointer"
                      >
                  ...
                  {role === "buyer" &&
                    ...
                    !disputedOrders.has(o.id) && (
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 bg-yellow-600 text-white text-[10px] font-mono rounded hover:bg-yellow-700 cursor-pointer"
                      >
                  ...
                  {disputedOrders.has(o.id) && (
                    <Link
                      to="/disputes"
                      className="flex items-center gap-1 px-2 py-1 bg-red-600/10 border border-red-600/20 text-red-500 text-[10px] font-mono rounded hover:bg-red-600 hover:text-white transition-all cursor-pointer font-bold"
                    >
                      Uyuşmazlığa Git
                    </Link>
                  )}
              ...
              <div className="mt-3">
                <OrderStatusTimeline
                  status={disputedOrders.has(o.id) ? "disputed" : o.status}
                />
              </div>
