...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (data) {
        ...
        const { data: pgp } = await (supabase as any)
          .from("user_pgp_keys")
          .select("public_key")
          .eq("user_id", data.vendor_id)
          .maybeSingle();
  ...
  if (loading)
    return (
      <PageShell>
        <div className="p-20 text-[10px] text-zinc-800 font-black animate-pulse">
          SİSTEM_VERİLERİ_YÜKLENİYOR...
        </div>
      </PageShell>
    );
  if (!product)
    return (
      <PageShell>
        <div className="p-20 text-red-600 font-black">VARLIK_BULUNAMADI</div>
      </PageShell>
    );
    ...
    <PageShell>
      <div className="max-w-[1200px] mx-auto py-8 space-y-12 font-mono">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="aspect-video bg-black rounded-[40px] overflow-hidden border-2 border-white/5 relative group">
              ...
              <div className="absolute top-6 left-6 z-20 flex gap-3">
                ...
                <div
                  className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest ${product.type === "digital" ? "bg-red-600 text-white" : "bg-white text-black"}`}
                >
            ...
            <div className="space-y-4">
              ...
              <p className="text-[11px] text-zinc-700 font-bold uppercase tracking-widest leading-relaxed max-w-xl">
                {product.description ||
                  "NO_PRODUCT_SPECIFICATIONS_FOUND_IN_CENTRAL_LOGS."}
              </p>
            </div>
          </div>
            ...
            <div className="obsidian-card p-8 rounded-[40px] border-2 border-white/5 space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-1">
                ...
                <div className="text-4xl font-black text-white italic tracking-tighter">
                  ...
                  <span className="text-xs text-red-600 not-italic ml-1">
                    LTC
                  </span>
                </div>
                <div className="flex items-center gap-4 text-[8px] text-zinc-800 font-black uppercase tracking-widest pt-2">
                  <span className="flex items-center gap-2">
                    <Shield className="w-3 h-3 text-red-900" /> ESCROW_ACTIVE
                  </span>
                  <span className="flex items-center gap-2">
                    <Activity className="w-3 h-3 text-red-900" /> STOCK:{" "}
                    {product.stock}
                  </span>
                </div>
              </div>
              ...
              {product.type === "physical" && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="text-[9px] text-zinc-800 font-black tracking-widest uppercase">
                        TESLİMAT_ADRESİ (PGP_ENCRYPTED)
                      </label>
                      {vendorPgp && (
                        <span className="text-[7px] text-red-600 font-black uppercase">
                          SECURE_PGP_LINK
                        </span>
                      )}
                    </div>
        ...
        <div className="border-t border-white/[0.04] pt-12 mt-12 space-y-8">
          <div className="space-y-2">
            <span className="text-[9px] text-zinc-500 font-bold tracking-[0.3em] uppercase">
              SYSTEM_FEEDBACK_LOGS
            </span>
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-[#030303]/30 border border-white/[0.04] p-6 rounded-[28px] space-y-6 relative overflow-hidden">
                ...
                <form onSubmit={handleSubmitReview} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest block">
                      Ürün Puanı
                    </label>
              ...
              {reviews.length === 0 ? (
                <div className="bg-[#030303]/20 border border-white/[0.02] p-12 rounded-[28px] text-center space-y-2">
                  ...
                  <p className="text-[9px] text-zinc-700 font-bold uppercase tracking-widest">
                    İlk yorumu siz yazarak deneyiminizi paylaşabilirsiniz.
                  </p>
                </div>
              ) : (
                reviews.map((r) => (
                  <div
                    key={r.id}
                    className="bg-[#020202]/30 border border-white/[0.03] rounded-[24px] p-6 space-y-4 hover:border-white/[0.08] transition-all duration-300"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
