...
import { 
  Camera, Save, User, Package, CheckCircle, 
  Loader2, Fingerprint, Zap, Shield, 
  Award, Target, Database, Key, Globe, Activity,
  Info, Eye, Edit3, Lock, Cpu, Sparkles, Terminal, Flame, ArrowUpRight
} from "lucide-react";
...
export default function Profile() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"identity" | "achievements" | "ledger" | "customization">("identity");
  const [bioTab, setBioTab] = useState<"edit" | "preview">("edit");

  // Level Up Customization Cosmetic States
  const [avatarBorder, setAvatarBorder] = useState(() => localStorage.getItem("profile_cosmetic_avatar_border") || "none");
  const [scanColor, setScanColor] = useState(() => localStorage.getItem("profile_cosmetic_scan_color") || "red");
  const [nameEffect, setNameEffect] = useState(() => localStorage.getItem("profile_cosmetic_name_effect") || "none");
  const [matrixBg, setMatrixBg] = useState(() => localStorage.getItem("profile_cosmetic_matrix_bg") || "none");
  ...
  const ACHIEVEMENTS = [
    { id: 1, label: "İLK_TEMAS", desc: "Sisteme başarılı giriş sağla.", icon: Globe, active: true },
    { id: 2, label: "DİJİTAL_GÖLGE", desc: "PGP anahtarını Vault'a yükle.", icon: Key, active: !!profile.pgp_key },
    { id: 3, label: "OPERATÖR", desc: "10 veya üzeri işlem hacmine ulaş.", icon: Activity, active: orders.length >= 10 },
    { id: 4, label: "VERİ_MADENCİSİ", desc: "En az 5 alışveriş tamamla.", icon: Database, active: orders.length >= 5 },
    { id: 5, label: "GÜVENLİ_LİMAN", desc: "Withdraw PIN kodunu aktif et.", icon: Shield, active: !!profile.withdraw_pin_hash },
    { id: 6, label: "EFSANE", desc: "Profil kodunu ve biyografini doldur.", icon: Award, active: !!profile.display_name && !!profile.bio },
  ];
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-zinc-500 max-w-xl uppercase font-bold tracking-wider leading-relaxed">
                Bu alandan genel profil kodunuzu, takma adınızı (Display Name), lokal şifrelenmiş biyografi metninizi yönetebilir ve PGP anahtarlarınızı güncelleyebilirsiniz.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="relative h-80 rounded-[40px] overflow-hidden border border-white/[0.05] bg-[#020202] shadow-2xl group">
              ...
              <div className={`absolute left-0 top-0 w-full h-[1.5px] z-10 animate-scan pointer-events-none ${
                scanColor === "green"
              ...
              <div className="absolute bottom-6 left-8 z-20 flex items-end gap-6">
                <div className="relative group/avatar">
                  ...
                  <div className={`absolute -inset-1 rounded-3xl blur-[8px] opacity-70 group-hover/avatar:opacity-100 transition-opacity animate-pulse ${
                  ...
                  <div className={`w-28 h-28 rounded-3xl bg-black border-2 overflow-hidden relative shadow-2xl z-10 ${
                    avatarBorder === "neon-red"
                      ? "border-red-500"
                      : avatarBorder === "neon-cyan"
                      ? "border-cyan-400"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 bg-[#040404]/55 backdrop-blur-xl p-1.5 border border-white/[0.04] rounded-2xl text-[9px] font-black uppercase tracking-wider shadow-md">
              {[
                { id: "identity", label: "💻 Kimlik Yapılandırması", desc: "Takma ad & Biyografi" },
                { id: "achievements", label: "🏆 Siber Başarımlar", desc: "Aktivite & Güvenlik ödülleri" },
                { id: "ledger", label: "📦 Sipariş & İşlem Defteri", desc: "Finansal intikal geçmişiniz" },
                { id: "customization", label: "🎨 Siber Arayüz Özelleştirici", desc: "Level ödülleri & kozmetikler" }
              ].map((subTab) => {
                ...
                return (
                  ...
                  >
                    <span className="font-black tracking-widest">{subTab.label}</span>
                    <span className={`text-[6px] tracking-wider font-bold ${isSelected ? "text-red-200" : "text-zinc-600"}`}>
                      {subTab.desc}
                    </span>
                  </button>
                );
              ...
              {activeSubTab === "identity" && (
                ...
                >
                  <div className="flex items-center gap-3 border-b border-white/[0.03] pb-4">
                    ...
                    <h2 className="text-sm font-black italic text-white uppercase tracking-tight">KİMLİK YAPILANDIRMA PANELİ</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">KOD ADINIZ (DISPLAY NAME)</label>
                    ...
                    {/* Node Role info card */}
                    <div className="space-y-1.5">
                      <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">GÜVENLİK YETKİ SEVİYESİ</label>
                      <div className="w-full bg-[#020202] border border-white/[0.04] rounded-xl px-4 py-3.5 text-xs text-primary font-black uppercase tracking-widest flex items-center justify-between">
                        <span>{role === "vendor" ? "VENDOR_OPERATOR" : "SECURED_BUYER"}</span>
                    ...
                    <div className="md:col-span-2 space-y-2">
                      <div className="flex items-center justify-between border-b border-white/[0.03] pb-1.5">
                        <label className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">KİMLİK BİYOGRAFİSİ (BIO_ENCRYPTION)</label>
                        ...
                        <div className="flex bg-[#020202] p-1 border border-white/[0.04] rounded-lg gap-1 text-[8px] font-black uppercase tracking-wider">
                          <button
                            ...
                            className={`px-3 py-1 rounded-md transition-all cursor-pointer ${
                              bioTab === "edit" ? "bg-red-600 text-white" : "text-zinc-500 hover:text-zinc-300"
                            }`}
                          >
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-md transition-all cursor-pointer ${
                              bioTab === "preview" ? "bg-red-600 text-white" : "text-zinc-500 hover:text-zinc-300"
                            }`}
                          >
                ...
                >
                  <div className="flex items-center justify-between border-b border-white/[0.03] pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-sm font-black italic text-white uppercase tracking-tight">KİMLİK BAŞARIMLARI</h2>
                    </div>
                  ...
                  <div className="p-5 bg-[#020202] border border-white/[0.03] rounded-2xl space-y-3">
                    <div className="flex justify-between items-center text-[9px] font-bold">
                      <span className="text-zinc-500 uppercase tracking-wider">KİMLİK TAMAMLANMA YÜZDESİ</span>
                      <span className="text-primary font-black tracking-widest">{progressPercent}% SECURE</span>
                    </div>
                    ...
                    {ACHIEVEMENTS.map((a) => (
                      ...
                      >
                        ...
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
                          a.active ? "bg-red-600 text-white shadow-md shadow-red-600/20" : "bg-white/5 text-zinc-700"
                        }`}>
                        ...
                        <div className="space-y-1">
                           <div className="font-black text-[9px] text-white uppercase tracking-wider flex items-center gap-1.5">
                             {a.label}
                             {a.active && <Sparkles className="w-3 h-3 text-amber-500 animate-pulse" />}
                           </div>
                           <div className="text-[7px] text-zinc-500 font-bold uppercase tracking-tight leading-none">{a.desc}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeSubTab === "ledger" && (
                ...
                >
                  <div className="flex items-center gap-3 border-b border-white/[0.03] pb-4">
                    ...
                    <h2 className="text-sm font-black italic text-white uppercase tracking-tight">ALICI SİPARİŞ GEÇMİŞİ</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                        ...
                        >
                          <div className="space-y-1.5">
                            ...
                            <div className="text-[7px] text-zinc-600 font-bold uppercase">
                              İŞLEM TARİHİ: {new Date(o.created_at).toLocaleDateString("tr-TR")}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-6 text-right">
                            ...
                            <div className="space-y-0.5">
                              <div className="text-[7px] text-zinc-500 font-bold uppercase">LTC DEĞERİ</div>
                              <div className="font-black text-emerald-500 italic text-xs">{o.amount.toFixed(4)} LTC</div>
                            </div>

                            <div className="hidden md:block space-y-0.5 border-l border-white/[0.04] pl-4">
                              <div className="text-[7px] text-zinc-600 font-bold uppercase">CANLI KUR EŞDEĞERİ</div>
                              <div className="text-[8px] font-bold space-x-1.5">
                                <span className="text-amber-500">{btcEquiv.toFixed(6)} BTC</span>
                                <span className="text-zinc-600">/</span>
                                <span className="text-red-400">{xmrEquiv.toFixed(4)} XMR</span>
                              </div>
                            </div>
                            ...
                            <span className={`px-2.5 py-1 rounded text-[8px] font-black uppercase tracking-widest ${
                              ...
                              {o.status === "completed" ? "TAMAMLANDI" : o.status === "delivered" ? "TESLİM EDİLDİ" : o.status.toUpperCase()}
                            </span>
                          </div>
                        </div>
                      );
                    ...
                    {orders.length === 0 && (
                      <div className="bg-[#040404]/55 border border-white/[0.04] p-16 rounded-[28px] text-center text-zinc-500 font-bold tracking-wider uppercase text-xs">
                        Henüz sisteme intikal eden alıcı siparişiniz bulunmamaktadır.
                      </div>
                    )}
                  </div>
                </motion.div>
                ...
                >
                  <div className="flex items-center justify-between border-b border-white/[0.03] pb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-sm font-black italic text-white uppercase tracking-tight">PROFİL ÖZELLEŞTİRME TERMİNALİ</h2>
                    </div>
                    <span className="text-[9px] font-black text-white bg-red-600/10 border border-red-500/20 px-3 py-1 rounded-lg">
                  ...
                  <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider leading-relaxed">
                    * SİSTEM BAŞARIMLARINI TAMAMLAYIP SEVİYE (LEVEL) ATLADIKÇA KİLİTLİ ARAYÜZ VE KOZMETİK ÖDÜLLERİNİ ALABİLİRSİNİZ. AŞAĞIDAN DİLEDİĞİNİZ KOZMETİĞİ SEÇEREK PROFİLİNİZİ SÜSLEYİN.
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#020202] p-6 rounded-3xl border border-white/[0.03] space-y-4">
                      <div className="text-[9px] text-zinc-500 font-black uppercase tracking-widest flex justify-between items-center">
                        ...
                        <span className="text-primary font-black">SEVİYE 2+ GEREKLİ</span>
                      </div>
                        ...
                        {[
                          ...
                          return (
                            <button
                              key={opt.id}
                              disabled={locked}
                              onClick={() => saveCosmetic("profile_cosmetic_avatar_border", opt.id, setAvatarBorder)}
                    ...
                    <div className="bg-[#020202] p-6 rounded-3xl border border-white/[0.03] space-y-4">
                      <div className="text-[9px] text-zinc-500 font-black uppercase tracking-widest flex justify-between items-center">
                        ...
                        <span className="text-primary font-black">SEVİYE 3+ GEREKLİ</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          return (
                            <button
                              key={opt.id}
                              disabled={locked}
                              onClick={() => saveCosmetic("profile_cosmetic_scan_color", opt.id, setScanColor)}
                              className={`py-3 rounded-xl border text-[8px] font-black tracking-widest uppercase transition-all ${
                                locked
                                  ? "opacity-30 border-white/5 text-zinc-700 cursor-not-allowed"
                                  : active
                    ...
                    <div className="bg-[#020202] p-6 rounded-3xl border border-white/[0.03] space-y-4">
                      <div className="text-[9px] text-zinc-500 font-black uppercase tracking-widest flex justify-between items-center">
                        ...
                        <span className="text-primary font-black">SEVİYE 4+ GEREKLİ</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          { id: "none", label: "STANDART", levelReq: 1 },
                          { id: "neon-glow", label: "RED PULSE", levelReq: 4 },
                          { id: "gold-pulse", label: "LEGEND GOLD", levelReq: 5 },
                        ].map((opt) => {
                          const locked = (1 + activeCount) < opt.levelReq;
                          ...
                          return (
                            <button
                              key={opt.id}
                              disabled={locked}
                              onClick={() => saveCosmetic("profile_cosmetic_name_effect", opt.id, setNameEffect)}
                              className={`py-3 rounded-xl border text-[8px] font-black tracking-widest uppercase transition-all ${
                                locked
                                  ? "opacity-30 border-white/5 text-zinc-700 cursor-not-allowed"
                                  : active
                    ...
                    <div className="bg-[#020202] p-6 rounded-3xl border border-white/[0.03] space-y-4">
                      <div className="text-[9px] text-zinc-500 font-black uppercase tracking-widest flex justify-between items-center">
                        ...
                        <span className="text-primary font-black">SEVİYE 5+ GEREKLİ</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          { id: "none", label: "STANDART", levelReq: 1 },
                          ...
                          return (
                            <button
                              key={opt.id}
                              disabled={locked}
                              onClick={() => saveCosmetic("profile_cosmetic_matrix_bg", opt.id, setMatrixBg)}
                              className={`py-3 rounded-xl border text-[8px] font-black tracking-widest uppercase transition-all ${
                                locked
                                  ? "opacity-30 border-white/5 text-zinc-700 cursor-not-allowed"
                                  : active
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-[#040404]/55 backdrop-blur-xl p-8 rounded-[36px] border border-white/[0.04] space-y-6 relative overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center gap-2.5 border-b border-white/[0.03] pb-3 text-[9px] text-primary font-black uppercase tracking-[0.2em]">
                 <Lock className="w-4 h-4 animate-pulse" /> SİBER GÜVENLİK TERMİNALİ
              </div>
            ...
            <div className="bg-[#040404]/55 backdrop-blur-xl p-8 rounded-[36px] border border-white/[0.04] space-y-6 shadow-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="bg-[#020202] p-5 rounded-2xl border border-white/[0.03] space-y-1 text-center hover:border-white/[0.06] transition-all">
                    ...
                    <div className="text-[7px] text-zinc-500 font-black uppercase leading-none">İŞLEM ADEDİ</div>
                 </div>
                 ...
                 <div className="bg-[#020202] p-5 rounded-2xl border border-white/[0.03] space-y-1 text-center hover:border-white/[0.06] transition-all">
                    ...
                    <div className="text-[7px] text-zinc-500 font-black uppercase leading-none">GÜVEN SKORU</div>
                 </div>
              ...
              <div className="p-4 bg-white/[0.01] border border-white/[0.03] rounded-xl flex gap-3 text-[8px] text-zinc-500 leading-normal uppercase">
                ...
                <div>
                  KİMLİK GÜVEN SKORUNUZU ARTIRMAK İÇİN PGP ANAHTARINIZI EKLEMEYİ VE GÜVENLİ PARA ÇEKME PIN ŞİFRENİZİ OLUŞTURMAYI UNUTMAYINIZ.
                </div>
              </div>
            </div>
