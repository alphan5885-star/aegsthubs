  ...
  return (
    <PageShell>
      <div className="max-w-[1100px] mx-auto py-8 space-y-12 font-mono">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 border-b border-white/5 pb-10">
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className="text-[8px] text-zinc-800 font-black uppercase tracking-widest">
                KAYIT_ADEDİ
              </div>
        ...
        {loading ? (
          <div className="p-10 text-[9px] text-zinc-800 font-black tracking-[0.4em] uppercase animate-pulse">
            SYNCHRONIZING_LEDGER...
          </div>
        ) : txs.length === 0 ? (
          <div className="obsidian-card p-12 rounded-[32px] border border-white/5 text-center space-y-3">
            <div className="text-xl font-black text-white uppercase tracking-tighter opacity-10">
              VERİ_YOK
            </div>
            <p className="text-[9px] text-zinc-800 font-bold uppercase tracking-widest leading-relaxed">
              SİSTEMDE_FİNANSAL_KAYIT_BULUNAMADI.
            </p>
          </div>
        ) : (
            ...
            {txs.map((tx, i) => (
              ...
              >
                <div className="flex items-center gap-5">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center border-2 transition-all ${tx.amount >= 0 ? "border-white/5 bg-white/5 text-white" : "border-red-600/20 bg-red-600/5 text-red-600"}`}
                  >
                    {tx.amount >= 0 ? (
                  ...
                  <div className="space-y-0.5">
                    <div className="text-lg font-black text-white uppercase tracking-tighter group-hover:text-red-600 transition-colors">
                      {tx.type.toUpperCase()}
                    </div>
                    <div className="text-[8px] text-zinc-800 font-bold uppercase tracking-widest leading-none">
                      {tx.description || "NO_DESC"} // ID: {tx.id.slice(0, 8)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between md:justify-end gap-10">
                  <div className="text-right">
                    <div className="text-[8px] text-zinc-900 font-black uppercase tracking-widest">
                      VALUE
                    </div>
                    <div
                      className={`text-2xl font-black italic tracking-tighter ${tx.amount >= 0 ? "text-white" : "text-red-600"}`}
                    >
                      {tx.amount >= 0 ? "+" : ""}
                      {tx.amount.toFixed(4)}{" "}
                      <span className="text-[10px] not-italic">LTC</span>
                    </div>
                  </div>
